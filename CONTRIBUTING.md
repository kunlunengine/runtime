...
The workspace requires Rust 1.85 or newer. The current bootstrap links Apple's system
`JavaScriptCore.framework` on macOS. The pinned, bundled JSC distribution for macOS and Linux is M1
work; `system-jsc` remains a developer-only backend.
